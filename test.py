...
def create_dual_semi_circular_progress(kill_progress_percent, death_progress_percent, filename='dual_progress_chart.png'):
    """Створює та зберігає подвійну напівкруглу діаграму прогресу Kills та Deaths."""
    ...
    # --- Внутрішній шар (Kills прогрес) ---
    radius_inner_kills = 0.8
    angle_kills = theta1 + (theta2 - theta1) * (kill_progress_percent / 100)
    ...
    # --- Середній шар (Deaths прогрес) ---
    radius_middle_deaths = 0.9
    angle_deaths = theta1 + (theta2 - theta1) * (death_progress_percent / 100)
...
async def commands(ctx):
    await ctx.send(
        "Available commands:\n"
        "!stats <Governor ID> - Get player statistics.\n"
        "!overview - Get overall statistics.\n"
        "!requirements - Check requirement completion.\n"
        "!top_dkp [limit=5] - Show top players by DKP.\n"
    )
...
async def req(ctx):
    try:
        ...
        if not not_completed.empty:
            await ctx.send("Players who have not met the requirements:")
            for index, row in not_completed.iterrows():
                await ctx.send(
                    f"{row['Governor Name']} (ID: {row['Governor ID']}) - "
                    f"Kills: {row['Kills Completion (%)']:.2f}%, "
                    f"Deaths: {row['Deaths Completion (%)']:.2f}%"
                )
        else:
            await ctx.send("All players have met the requirements!")
...
async def overview(ctx):
    try:
        ...
        await ctx.send(
            ...
            f"Average Kills Change: {average_kills:.2f}\n"
            f"Average Deaths Change: {average_deads:.2f}\n"
            f"Average Kill Completion (%): {average_completion_kills:.2f}%\n"
            f"Average Death Completion (%): {average_completion_deads:.2f}%"
        )
    except Exception as e:
        await ctx.send(f"An error occurred: {str(e)}")
...
@bot.command()
async def top_dkp(ctx, limit: int = 5):
    try:
        result = pd.read_excel('results.xlsx')
        result_sorted = result.sort_values(by='DKP', ascending=False).head(limit)
        embed = discord.Embed(title=f"🏆 Top {limit} Players by DKP", color=discord.Color.gold())
        for index, row in result_sorted.iterrows():
            embed.add_field(name=f"{index + 1}. {row['Governor Name']}", value=f"DKP: {round(row['DKP']):,}", inline=False)
        await ctx.send(embed=embed)
    except Exception as e:
        await ctx.send(f"An error occurred while processing the !top_dkp command: {str(e)}")
